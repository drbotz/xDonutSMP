import { AttachmentBuilder, BaseGuildTextChannel, ButtonInteraction, GuildMember, GuildTextBasedChannel, PermissionsBitField } from "discord.js";
...
export default {
    ...
    function: async function ({ button }: { button: ButtonInteraction }) {
        if (!button.inCachedGuild()) return await button.reply({ content: ":x: **You must be in the server to use this button!**", ephemeral: true });
        if (!button.member.permissions.has(PermissionsBitField.Flags.Administrator)) return await button.reply({ content: ":x: **You must be an administrator to use this button!**", ephemeral: true });
        ...
        const reportsChannel = await button.guild?.channels.fetch(config.channels.reports) as GuildTextBasedChannel;
        ...
        const quickSearchChannel = await button.guild?.channels.fetch(config.channels.quickSearch) as GuildTextBasedChannel;
        const oldMessage = await database.manager.findOne(Message, { where: { scammer: report.scammerUUID } });
        let oldMessageMsg;
        if (oldMessage) {
            const oldMessageChannel = await button.guild?.channels.fetch(oldMessage.channel).catch(() => null) as GuildTextBasedChannel;
        ...
        const channel = button.guild.channels.cache.get(config.tickets.transcriptsChannel) as BaseGuildTextChannel;
        ...
        if (!ticket) return await button.channel.delete();
        ...
        const member: GuildMember = await button.guild.members.fetch(report.scammer).catch(() => null);
        if (member) await member.roles.set([config.scammerRole]);
