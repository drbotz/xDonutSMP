...
export async function setBusy(state: boolean) {
    busy = state;
}
...
const formatItems = (items: any) => {
    return items.map(item => {
        return {
            ...
            lore: item.customLore.map((line: any) => JSON.parse(line).extra?.map((line: any) => line.text) || JSON.parse(line)?.text || "").flat(),
            slot: item.slot,
            stack: item.count
        }
    });
...
export async function minecraft() {
    bot = mineflayer.createBot(config.minecraft as any);

    bot.on("message", async (message) => {
        if (config.logMCChat) console.log(`${colors.green("[Minecraft]")} ${message.toAnsi()}`);
        ...
        if (balMatch?.length) {
            bot.emit("bal" as any, balMatch.groups.balance);
        }
        if (message.toString().match(rejectionRegexShards)) {
            bot.emit("shards" as any, "ENOENT");
        }
        if (message.toString().match(rejectionRegexBal)) {
            bot.emit("bal" as any, "ENOENT");
        }
        if (paidMatch?.length) {
            let amount = parseInt(paidMatch.groups.amount);

            if (paidMatch.groups.suffix.toLowerCase() == "k") amount *= 1000;
            if (paidMatch.groups.suffix.toLowerCase() == "m") amount *= 1000000;
            if (paidMatch.groups.suffix.toLowerCase() == "b") amount *= 1000000000;

            bot.emit("paid" as any, paidMatch.groups.username, amount, paidMatch.groups.suffix);
        }

        const shardsRegex = /\w+'\w+ shards: (?<shards>[\w\.M]+)/gm;
        const shardsMatch = shardsRegex.exec(message.toString());
        if (shardsMatch?.length) {
            bot.emit("shards" as any, shardsMatch.groups.shards);
        }

        const notOnlineMatch = message.toString().match(/This user is not online./gm);
        if (notOnlineMatch?.length) {
            bot.emit("online" as any, false);
        }
        ...
        if (codeMatch?.length) {
            ...
            if (codeData.ign.toLowerCase() != username.toLowerCase()) return;
            if (codeData.expires < Date.now()) return bot.chat(`/w ${username} Your code has expired!`);
            ...
            if (alreadyLinked) await database.manager.remove(Link, alreadyLinked);
            ...
            const stats = await getStats(codeData.ign) as any;
            const moneyMade = stats.find(stat => stat.stat == "money made")?.value || "0";
            let parsedAmount = parseInt(moneyMade.replace(/[^0-9]/g, ""));
            if (moneyMade.endsWith("K")) parsedAmount *= 1000;
            if (moneyMade.endsWith("M")) parsedAmount *= 1000000;
            if (moneyMade.endsWith("B")) parsedAmount *= 1000000000;
        ...
        if (!balMatch?.length && !paidMatch?.length && !rejectionRegexShards.test(message.toString()) && !rejectionRegexBal.test(message.toString()) && !chatMatch?.length) {
            bot.emit("commandResponse" as any, message.toString());
        }
    });
    ...
    bot.on("windowOpen", (window) => {
        if (window.title.includes("ѕᴛᴀᴛѕ")) {
            ...
            for (const stat of stats) {
                bot.emit(stat.stat as any, stat.value);
            }

            bot.emit("stats" as any, stats);
        }
        ...
        if (window.title.includes("ᴄᴏɴꜰɪʀᴍ ʀᴇǫᴜᴇѕᴛ")) {
            const items = window.containerItems().map(item => {
                return {
                    ...
                    desription: JSON.parse(item.customLore[0]).extra.map((line: any) => line.text).join("\n")
                }
            });

            bot.emit("online" as any, true, items.find(item => item.customName == "ʟᴏᴄᴀᴛɪᴏɴ").desription);
        }
        ...
        for (const [key, value] of Object.entries(leaderboardTitles)) {
            if (window.title.includes(value)) {
                ...
                bot.emit("leaderboard" as any, key, items);
            }
        }
    });
    ...
    bot.once("login", async () => {
        ...
        setInterval(() => {
            if (messages.length) {
                (client.channels.cache.get(config.discordSRV.channel) as GuildTextBasedChannel)?.send(messages.map(message => `**${message.username}**: ${message.text}`).join("\n"));
                messages = [];
            }
        }, 5000);
        ...
        const takeFeeJob = new CronJob("0 0 0 * * 0", async () => {
            ...
            for await (const fee of fees) {
                ...
                if (!bank || bank.balance < fee.amount) {
                    const channel = client.channels.cache.get(config.bank.logsChannel) as GuildTextBasedChannel;
    ...
    bot.on("windowOpen", async (window) => {
        if (!window.title.includes("ᴄᴏɴꜰɪʀᴍ ᴘᴜʀᴄʜᴀѕᴇ")) return;
        ...
        const channel = client.channels.cache.get(config.channels.ahSnipes) as GuildTextBasedChannel;
        channel.send({ embeds: [embed] });
    });

    bot.on("paid" as any, async (username, amount, suffix) => {
...
export async function getBal(ign: string): Promise<number> {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/bal ${ign}`);
        bot.once("bal" as any, (message) => {
            busy = false;
            if (message == "Player not found") reject("Player not found");
            resolve(message);
        });
        setTimeout(() => {
            busy = false;
...
export async function getShards(ign: string): Promise<number> {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/shards ${ign}`);
        bot.once("shards" as any, (message) => {
            busy = false;
            resolve(message);
        });
        setTimeout(() => {
...
export async function getKills(ign: string) {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/stats ${ign}`);
        bot.once("kills" as any, (message) => {
            busy = false;
            resolve(message);
        });
        setTimeout(() => {
...
export async function getDeaths(ign: string) {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/stats ${ign}`);
        bot.once("deaths" as any, (message) => {
            busy = false;
            resolve(message);
        });
        setTimeout(() => {
...
export async function getPlaytime(ign: string) {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/stats ${ign}`);
        bot.once("playtime" as any, (message) => {
            busy = false;
            resolve(message);
        });
        setTimeout(() => {
...
export async function isOnline(ign: string): Promise<{ status: Boolean, location: string }> {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/tpa ${ign}`);
        bot.once("online" as any, (status, location) => {
            busy = false;
...
export async function getStats(ign: string) {
    busy = true;
    return new Promise((resolve, reject) => {
        bot.chat(`/stats ${ign}`);
        bot.once("stats" as any, (message) => {
            busy = false;
            resolve(message);
        });
        setTimeout(() => {
...
export async function getLeaderboard(leaderboard: string): Promise<{ leaderboard: string, users: { name: string, lore: string, slot: number }[] }> {
    busy = true;
    return new Promise((resolve, reject) => {
        ...
        bot.once("windowOpen", (window) => {
            ...
            if (leaderboardItem) bot.clickWindow(leaderboardItem.slot, 0, 0);
        });

        bot.once("leaderboard" as any, (leaderboard: string, users: { name: string, lore: string, slot: number }[]) => {
            busy = false;
...
export async function executeCommand(cmd: string) {
    bot.chat(cmd);
    return new Promise((resolve, reject) => {
        bot.once("commandResponse" as any, (message) => {
            resolve(message);
        });
...
async function broke() {
    return new Promise((resolve, reject) => {
        ...
        bot.once("message", (message) => {
            if (message.toString().includes("You don't have enough money for this.")) return resolve(true);
        });
    });
}

export async function checkAh(itemName: string, stack: number = 1) {
    ...
    bot.once("windowOpen", async (window) => {
        ...
        if (lowestPrice.lore[1].endsWith("k")) price *= 1000;
        if (lowestPrice.lore[1].endsWith("m")) price *= 1000000;
        if (lowestPrice.lore[1].endsWith("b")) price *= 1000000000;
