...
export default {
    ...
    function: async function ({ message, args }: { message: Message, args: string[] }) {
        ...
        if (!userProvided && !idProvided) return message.channel.send(":x: **Please provide a user to link!**");
        if (!ign) return message.channel.send(":x: **Please provide an IGN to link!**");

        let user;

        if (userProvided) user = userProvided;
        else if (idProvided) user = await message.guild.members.fetch(idProvided).catch(() => null);
        ...
        if (alreadyLinked) await database.manager.delete(Link, { discordId: user.id });
        ...
        if (linked) {
            ...
            const member = await message.guild.members.fetch(user.id).catch(() => null) as GuildMember;
            if (member) await member.setNickname(`${member.user.displayName} [${ign}]`).catch(() => null);
        } else {
