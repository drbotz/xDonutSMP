import { GuildMember, GuildTextBasedChannel, Message as Msg } from "discord.js";
...
export default {
    ...
    usage: "delete <id>",
    function: async function ({ message, args }: { message: Msg, args: string[] }) {
        const id = parseInt(args[0]);
        if (!id) return message.channel.send(":x: **You must provide a report ID!**");

        const report = await database.manager.findOne(Scammer, { where: { id } });
        if (!report) return message.channel.send(":x: **That report doesn't exist!**");
        ...
        let oldMessageMsg;
        if (oldMessage) {
            const oldMessageChannel = await message.guild?.channels.fetch(oldMessage.channel).catch(() => null) as GuildTextBasedChannel;
        ...
        if (oldMessageMsg) {
            ...
            else await oldMessageMsg.edit({ content: `${report.scammerIGN} (${report.scammerUUID}) - ${scammer.map(s => `[${s.id}](${s.messageURL})`)}` });
        }

        const channel = await message.guild?.channels.fetch(config.channels.reports) as GuildTextBasedChannel;
        const msg = await channel.messages.fetch(report.messageURL.split("/").pop()).catch(() => null);
        await msg.delete();

        if (!scammer.length) {
            const member: GuildMember = await message.guild?.members.fetch(report?.scammerDiscord).catch(() => null);
            if (member) await member.roles?.set(config.joinRoles);
        }
