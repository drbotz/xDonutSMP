...
export default {
    ...
    function: async function ({ message, args }: { message: Message, args: string[] }) {
        ...
        if (!roles.some(role => message.member.roles.cache.has(role)) && !message.member.permissions.has(PermissionsBitField.Flags.Administrator)) return message.channel.send(":x: **You do not have permission to remove users from this ticket!**");

        const user = message.mentions.users.first() || message.guild.members.cache.get(args[0]);
        if (!user) return message.channel.send(":x: **Please provide a user to remove!**");

        if (!ticket.added.includes(user.id)) return message.channel.send(":x: **This user is already not in the ticket!**");

        if (message.channel.type == ChannelType.GuildText) await message.channel.permissionOverwrites.edit(user.id, { ViewChannel: false, SendMessages: false });
