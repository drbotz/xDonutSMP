...
export default {
    ...
    function: async function({ message }: { message: Message }) {
        ...
        for (const member of members.values()) {
            ...
            if (scammer) {
                const scammerRole = await message.guild.roles.fetch(config.scammerRole).catch(() => null);
                if (scammerRole) await member.roles?.add(scammerRole).catch(() => null);
            }

            const stats = await getStats(link.ign) as any;
            ...
            if (wealthRole) {
                const role = message.guild?.roles.cache.get(wealthRole.role);
                await member?.roles.add(role);
            }
        }

        await message.channel.send(":white_check_mark: **Successfully synced up all the scammer roles, linked roles, and nicknames.**");
    }
}