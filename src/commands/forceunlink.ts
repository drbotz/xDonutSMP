...
export default {
    ...
    function: async function ({ message, args }: { message: Message, args: string[] }) {
        ...
        if (!userProvided && !idProvided) return message.channel.send(":x: **Please provide a user to unlink!**");
        ...
        else await message.channel.send(`:x: **${user.tag} is not linked to any account**`);
    }
}