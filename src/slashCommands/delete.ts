import { ApplicationCommandOptionType, ChatInputCommandInteraction, GuildMember, GuildTextBasedChannel } from "discord.js";
...
export default {
    ...
    function: async function ({ interaction }: { interaction: ChatInputCommandInteraction }) {
        ...
        if (!report) return await interaction.reply({ content: ":x: **That report doesn't exist!**", ephemeral: true });
        ...
        let oldMessageMsg;
        if (oldMessage) {
            const oldMessageChannel = await interaction.guild?.channels.fetch(oldMessage.channel).catch(() => null) as GuildTextBasedChannel;
        ...
        const channel = await interaction.guild?.channels.fetch(config.channels.reports) as GuildTextBasedChannel;
        const msg = await channel.messages.fetch(report.messageURL.split("/").pop()).catch(() => null);
        await msg.delete();

        if (!scammer.length) {
            const member: GuildMember = await interaction.guild?.members.fetch(report?.scammerDiscord).catch(() => null);
