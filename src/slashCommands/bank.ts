import { ApplicationCommandOptionType, ChatInputCommandInteraction, OverwriteResolvable } from "discord.js";
...
export default {
    ...
    function: async function({ interaction }: { interaction: ChatInputCommandInteraction }) {
        ...
        if (!linked) return interaction.reply({ content: ":x: **You are not linked to any account!**", ephemeral: true });
        ...
        if (subcommand == "deposit") {
            ...
            if (type == "money") {
                ...
                if (!requiredRole) return interaction.reply({ content: ":x: **The physical role is not set up!**", ephemeral: true });
                if (requiredRole.position > interaction.member.roles.highest.position) return interaction.reply({ content: ":x: **You do not have the required role to deposit physical money!**", ephemeral: true });
            ...
            if (type == "money") {
                ...
                if (balance < amount) return interaction.reply({ content: ":x: **You do not have enough money in your account!**", ephemeral: true });
                ...
                const requiredRole = interaction.guild.roles.cache.get(config.bank.physicalRole);
                if (!requiredRole) return interaction.reply({ content: ":x: **The physical role is not set up!**", ephemeral: true });
                if (requiredRole.position > interaction.member.roles.highest.position) return interaction.reply({ content: ":x: **You do not have the required role to withdraw physical money!**", ephemeral: true });
