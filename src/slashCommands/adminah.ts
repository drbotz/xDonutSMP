...
export default {
    ...
    function: async function({ interaction }: { interaction: ChatInputCommandInteraction }) {
        ...
        if (subcommand === "add") {
            ...
            if (alreadyExists) return await interaction.reply({ content: `:x: **This item already exists with the price: $${alreadyExists.price.toLocaleString()}.**`, ephemeral: true });
            ...
            if (!item) return await interaction.reply({ content: `:x: **This item doesn't exist in the auction house sniper.**`, ephemeral: true });
            ...
            if (!items.length) return await interaction.reply({ content: `:x: **There are no items in the auction house sniper.**`, ephemeral: true });
