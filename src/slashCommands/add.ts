...
export default {
    ...
    function: async function ({ interaction }: { interaction: ChatInputCommandInteraction }) {
        ...
        if (ticket.added.includes(user.id)) return interaction.reply({ content: ":x: **This user is already in the ticket!**", ephemeral: true });

        if (!interaction.channel.isThread()) await interaction.channel.permissionOverwrites.edit(user.id, { ViewChannel: true, SendMessages: true });
