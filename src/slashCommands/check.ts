...
export default {
    ...
    function: async function ({ interaction }: { interaction: ChatInputCommandInteraction }) {
        ...
        if (!ign && !mention && !userid) return interaction.reply({ content: ":x: **You must provide a user IGN, mention or ID!**", ephemeral: true });

        if (ign) {
            const uuid = await getUUID(ign);
            if (!uuid) return interaction.reply({ content: ":x: **That user doesn't exist!**", ephemeral: true });

            const scammer = await database.manager.find(Scammer, { where: { scammerUUID: uuid } });
            if (!scammer.length) return interaction.reply({ content: ":x: **That user isn't a scammer!**", ephemeral: true });
