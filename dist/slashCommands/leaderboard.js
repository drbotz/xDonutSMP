...
export default {
    ...
    function: async function ({ interaction }) {
        ...
        if (!type && !username)
            return interaction.editReply({ content: ":x: **Please specify a leaderboard type or a username!**" });
        if (!type && username) {
            const results = [];
            ...
            if (username) {
                ...
                if (!user)
                    return interaction.editReply(":x: **User not found on the leaderboard!**");
