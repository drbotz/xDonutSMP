...
export default {
    ...
    function: async ({ interaction }) => {
        ...
        if (!description && !proof && !scammerDiscord)
            return interaction.reply({ content: ":x: **You must provide a description, proof or the scammer's discord!**", ephemeral: true });
        const report = await database.manager.findOne(Scammer, { where: { id } });
        if (!report)
            return interaction.reply({ content: ":x: **That report doesn't exist!**", ephemeral: true });
        const scammer = await database.manager.findOne(Scammer, { where: { scammerUUID: report.scammerUUID, date: report.date } });
        if (!scammer)
            return interaction.reply({ content: ":x: **That report doesn't exist!**", ephemeral: true });
        if (description)
            await database.manager.update(Scammer, { id }, { description });
        if (proof)
            await database.manager.update(Scammer, { id }, { proof });
        if (scammerDiscord)
            await database.manager.update(Scammer, { id }, { scammerDiscord });
        const channel = await interaction.client.channels.fetch(config.channels.reports);
        if (!channel)
            return interaction.reply({ content: ":x: **That report doesn't exist!**", ephemeral: true });
