            ...
            switch (config.reportQuestions.indexOf(question)) {
                case ReportQuestionsIndex.reporterIGN:
                    ...
                    if (reporterUUID === null)
                        await button.reply({ content: ":x: **The IGN you entered is invalid. Please try again.**", ephemeral: true });
                    report = report.replace(/{reporter}/g, button.user.tag);
                ...
                case ReportQuestionsIndex.scammerIGN:
                    ...
                    if (scammerUUID === null)
                        return await button.reply({ content: ":x: **The scammer IGN you entered is invalid. Please try again.**", ephemeral: true });
                    scammerIGN = answer;
                ...
                case ReportQuestionsIndex.scammerDiscord:
                    ...
                    if (!parseInt(answer) && answer.length < 17)
                        return await button.reply({ content: ":x: **The scammer Discord ID you entered is invalid. Please try again.**", ephemeral: true });
