...
export default {
    ...
    function: async function ({ message, args }) {
        ...
        if (!reason)
            return message.channel.send(":x: **You must provide a reason!**");
        const ticket = await database.manager.findOne(Ticket, { where: { channel: message.channel.id } });
        if (!ticket)
            return message.reply({ content: ":x: **This is not a ticket channel!**" });
        ...
        const channel = message.guild.channels.cache.get(config.tickets.transcriptsChannel);
