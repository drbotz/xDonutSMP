...
export default {
    ...
    function: async function ({ message, args }) {
        ...
        if (!ign && !mention && !userid)
            return message.channel.send(":x: **You must provide a user IGN, mention or ID!**");
        if (ign) {
            const uuid = await getUUID(ign);
            if (!uuid)
                return message.channel.send(":x: **That user doesn't exist!**");
            const scammer = await database.manager.find(Scammer, { where: { scammerUUID: uuid } });
            if (!scammer.length)
                return message.channel.send(":x: **That user isn't a scammer!**");
