    ...
    function: async function () {
        ...
        setInterval(async () => {
            ...
            for (const channel of channels) {
                if (Date.now() - channel.createdAt > 86400000) {
                    const questionChannel = await client.channels.fetch(channel.channelId).catch(() => null);
                    if (questionChannel) {
                        ...
                        const transcriptsChannel = await client.channels.fetch(config.tickets.transcriptsChannel).catch(() => null);
        ...
        const checkLeaderboards = async () => {
            ...
            for (const leaderboard of leaderboards) {
                const channel = await client.channels.fetch(leaderboard.channelId).catch(() => null);
                if (!channel)
                    continue;
