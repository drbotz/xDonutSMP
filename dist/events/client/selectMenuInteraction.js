...
export default {
    ...
    function: async function (interaction) {
        ...
        logsChannel.send({
            ...
            embeds: [embed.setDescription(`**Select Menu Clicked**\n\n**Select Menu:** ${interaction.customId}\n**Author:** ${interaction.user.tag} (${interaction.user.id})\n**Server:** ${interaction.guild.name}`)]
        });
        ...
        if (selectMenu.cooldown !== 0 && (interaction.guild && memberPerms.has(PermissionsBitField.Flags.Administrator))) {
            ...
            setTimeout(() => {
                const index = client.cooldowns.findIndex((cooldown) => cooldown.user === interaction.user.id && cooldown.id === selectMenu.id);
                if (index !== -1) {
