        ...
        if (command) {
            if (command.permissions?.length) {
                ...
                if (invalidPerms.length)
                    return message.channel.send(`Missing Permissions: \`${invalidPerms + "".replace(/,/g, ", ")}\``);
            }
            if (command.roleRequired) {
                const role = await message.guild?.roles.fetch(command.roleRequired);
                if (role && message.member && !message.member?.roles.cache.has(role.id) && message.member.roles.highest.comparePositionTo(role) < 0 && !message.member.permissions.has(PermissionsBitField.Flags.Administrator))
                    return message.channel.send(`:x: **You don't have the required role!**`);
            }
