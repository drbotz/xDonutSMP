...
export default {
    ...
    function: async function (message) {
        ...
        if (message.channel.type == ChannelType.DM)
            return;
        if (!message.channel.name.startsWith("answering-"))
            return;
        ...
        if (!nextQuestion) {
            ...
            if (botMessages.find(m => m.content.includes("Would you like to submit the questions?")))
                return;
                ...
                switch (config.reportQuestions.indexOf(question)) {
                    case ReportQuestionsIndex.reporterIGN:
                        ...
                        if (reporterUUID === null)
                            await sendTempMessage(":x: **The IGN you entered is invalid. Please try again.**");
                        report = report.replace(/{reporter}/g, message.author.tag);
                    ...
                    case ReportQuestionsIndex.scammerIGN:
                        ...
                        if (scammerUUID === null)
                            return await sendTempMessage(":x: **The scammer IGN you entered is invalid. Please try again.**");
                        scammerIGN = answer;
                    ...
                    case ReportQuestionsIndex.scammerDiscord:
                        ...
                        if (!parseInt(answer) && answer.length < 17)
                            return await sendTempMessage(":x: **The scammer Discord ID you entered is invalid. Please try again.**");
            ...
            const row = new ActionRowBuilder().addComponents(buildButton("submit"), buildButton("restart"), buildButton("cancel"));
            ...
            switch (currentQuestionIndex) {
                case ReportQuestionsIndex.reporterIGN:
                    ...
                    uuidCache.set(message.content, reporterUUID);
                    if (reporterUUID === null)
                        return await sendTempMessage(":x: **The IGN you entered is invalid. Please try again.**");
                    break;
                case ReportQuestionsIndex.scammerIGN:
                    if (message.content?.toLowerCase() == "n/a")
                        break;
                ...
                case ReportQuestionsIndex.scammerDiscord:
                    if (message.content?.toLowerCase() == "n/a")
                        break;
                    if (!parseInt(message.content) && message.content.length < 17)
                        return await sendTempMessage(":x: **The scammer Discord ID you entered is invalid. Please try again.**");
                    break;
                case ReportQuestionsIndex.scamDescription:
                    break;
